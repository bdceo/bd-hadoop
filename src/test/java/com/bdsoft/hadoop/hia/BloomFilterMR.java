...
public class BloomFilterMR extends Configured implements Tool {
	...
	public static class MapClass extends MapReduceBase implements
			Mapper<Text, Text, Text, BloomFilter<String>> {

		BloomFilter<String> bf = new BloomFilter<String>();
		// 内部持有输出对象引用，在close中进行最终输出
		...
		public void close() throws IOException {
			oc.collect(new Text("someKey"), bf);
		}
	}
	...
	public int run(String[] args) throws Exception {
		...
		job.setJobName(BloomFilterMR.class.getName());
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner
				.run(new Configuration(), new BloomFilterMR(), args);
		System.exit(res);
	}
